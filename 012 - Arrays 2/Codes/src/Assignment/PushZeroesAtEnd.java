 ...
 * Another important fact is that the order of the non-zero elements have been maintained as they
 * appear in the input array/list.
 *
 * -----------------------------------
 * Sample Input 2:
 * 2
...
public class PushZeroesAtEnd {
    ...
    public static void pushZerosAtEnd(int[] arr) {
        ...
        // for loop for loop transverse
        for (int i = 0 ; i< arr.length ; i++  ){
            if (arr[i]!=0){
                int temp = arr[i];
                arr[i]=0;
                arr[zeroIndexPosition]=temp;
                zeroIndexPosition++;
            }
