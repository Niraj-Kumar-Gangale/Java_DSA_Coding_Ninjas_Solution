...
public class TotalSumOnBoundariesAndDiagonals {
    ...
    public static void totalSum(int[][] mat) {
        ...
        // 7 8 9
        int totalSum = 0;
        // sum of the diagonal element element ie 1 5 9
        for (int i = 0; i < mat.length ; ++i){
            if (i==(mat.length-1-i)){
                totalSum += mat[i][i];
                continue;
            }
            totalSum += mat[i][i] + mat[i][mat.length-1-i];
        }

        // sum of the boundaries elements
        for (int i = 1; i < mat.length-1 ; ++i){
            totalSum += mat[0][i] + mat[i][0] + mat[mat.length-1][i] + mat[i][mat.length-1];
        }
